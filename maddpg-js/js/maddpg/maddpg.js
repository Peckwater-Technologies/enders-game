...
// This class is called from js/MADDPG/ddpg_agent.js
class MADDPG {
    ...
    constructor(actor, critic, memory, noise, config){
        ...
        this.otherActionInput = tf.input({batchShape: [null, this.config.nbOtherActions]});
        // Randomly Initialize actor network μ(s)
        this.actor.buildModel(this.obsInput);
        // Randomly Initialize critic network Q(s, a)
        this.critic.buildModel(this.obsInput, tf.concat([this.actionInput,this.otherActionInput], axis=1));
    ...
    getQvalue(state, a, oa){
        ...
        const tfoa = tf.tensor2d([oa]);
        const q = this.critic.model.predict([st, tf.concat([tfa, tfoa],axis = 1)]);
    ...
    trainActor(tfObs0){
        ...
        const actorLoss = this.actorOptimiser.minimize(() => {
            const tfQPredictions0 = tf.concat([this.criticWithActor(tfObs0), this.otherActionInput], axis=1); 
